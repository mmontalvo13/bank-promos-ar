...
import type { PromotionGroup } from "@/lib/promotions/group";
...
function eligibilityLabel(p: Promotion) {
  const seg = p.eligibility?.segment ?? null;
  if (seg) return seg;
  if (p.eligibility?.eminent) return "Eminent";
  if (p.eligibility?.haberes) return "Haberes";
  return "General";
}

export function PromotionGroupCard({ group }: { group: PromotionGroup }) {
  const top = group.variants[0];
  const titleWithoutPercent = top.title.replace(/^\s*\d{1,2}\s?%\s*·\s*/i, "");
  const promoUrl =
    top.source?.url ??
    (top.bank === "MODO" ? "https://www.modo.com.ar/promos" : null);
      ...
      <summary className="cursor-pointer list-none p-4 [&::-webkit-details-marker]:hidden">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-xs font-semibold text-neutral-700">{displayBankOrWallet(top)}</div>
            <div className="mt-0.5 line-clamp-1 text-sm font-extrabold text-neutral-950">{group.store}</div>
          </div>
        ...
        <h3 className="mt-3 line-clamp-2 text-sm font-bold leading-snug text-neutral-950">{titleWithoutPercent}</h3>

        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          <Badge className="bg-neutral-50 text-neutral-900">
            {isEveryDay(top.days) ? "Todos los días" : top.days.map(dayLabel).join(" · ")}
          </Badge>
          {top.capArs ? (
            <Badge className="border-neutral-200 bg-neutral-50 text-neutral-800">
              Tope: ${top.capArs.toLocaleString("es-AR")}
            </Badge>
          ) : null}
        </div>
          ...
          {group.paymentMethods.map((m) => {
            ...
            return (
              <span
                ...
                title={paymentLabelForCard(top, m)}
              >
                <Icon className="h-3.5 w-3.5" />
                {paymentLabelForCard(top, m)}
              </span>
            );
          })}
        </div>
      ...
      <div className="border-t border-neutral-200 px-4 pb-4 pt-3">
        ...
        {group.variants.length > 1 ? (
          <div className="mt-3 rounded-2xl border border-neutral-200 bg-neutral-50 p-3">
            <div className="text-xs font-extrabold uppercase tracking-wide text-neutral-700">Opciones según tu cuenta</div>
            <div className="mt-2 space-y-2">
              {group.variants.map((v) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-sm font-extrabold text-neutral-950">{v.discountPercent}%</div>
                    <div className="text-xs font-semibold text-neutral-600">{eligibilityLabel(v)}</div>
                  </div>
                  {v.capArs ? (
                    <div className="text-xs font-extrabold text-neutral-800">Tope ${v.capArs.toLocaleString("es-AR")}</div>
                  ) : null}
                </div>
              ))}
            </div>
        ...
        {top.notes ? <p className="mt-3 text-sm leading-relaxed text-neutral-700">{top.notes}</p> : null}
